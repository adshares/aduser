...
.. code-block:: ini
    ...
    [program:aduser]
    directory=%(ENV_aduser_ROOT)s
    command=pipenv run python daemon.py
    pidfile=%(ENV_aduser_ROOT)s/aduser.pid
    stdout_logfile=%(ENV_aduser_ROOT)s/aduser.log
    stdout_logfile_maxbytes=50MB
    stdout_logfile_backups=10
    redirect_stderr=true

ENV_aduser_ROOT is the path to the project directory.
...
.. code-block:: docker
    ...
    RUN git clone https://github.com/adshares/aduser.git /aduser
    WORKDIR /aduser
